...
class Auth0 {
  ...
  setSession(authResult) {
    let expriresAt = JSON.stringify(
      authResult.expiresIn * 1000 + new Date().getTime()
    );
    console.log(authResult.expiresIn);

    Cookies.set("x-jwt-exp", expriresAt);
  ...
  async getJWST() {
    const res = await axios.get(
      "https://dev-c9tibd3j.auth0.com/.well-known/jwks.json"
    );
    const jwks = res.data;
    return jwks;
  }
  ...
  async isAuthenticated(req) {
    if (process.browser) {
      ...
      let token = getCooksFromReq(req, "x-jwt-exp");
      const verifyToken = await this.verifyToken(token);
